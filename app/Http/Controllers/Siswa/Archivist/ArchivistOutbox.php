...
class ArchivistOutbox extends Controller
{
    ...
    {
        $group_id = UserGroup::select('group_id')->where('user_id', Auth::id())->first();
        $classification = Classification::select('*')->where('group_id', $group_id->group_id)->get();
    ...
    public function editOutbox(Request $request)
    {
        ...
        return view('siswa.archivist.edit_outbox')
            ->with('concept', $data)
            ->with('mail', $mail)
            ->with('nama', auth()->user()->name);
    }
    ...
    public function updateOutbox(Request $request)
    {
        ...
        if (empty($request->logo)) {
            ...
            return redirect(route('viewMail_arc', $request->id));
        } else {
    ...
    public function deleteOutbox(Request $request)
    {
        ...
        return redirect(route('outbox_data_arc'));
    }
}
